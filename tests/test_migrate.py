...
def test_pack_and_restore_bundle(tmp_path: Path) -> None:
    ...
    (src / "state" / "runs").mkdir(parents=True, exist_ok=True)
    ...
    (src / "state" / "runs" / f"{run_id}.json").write_text(json.dumps({"run_id": run_id}))
    ...
    assert f"state/runs/{run_id}.json" in restored["restored"]
